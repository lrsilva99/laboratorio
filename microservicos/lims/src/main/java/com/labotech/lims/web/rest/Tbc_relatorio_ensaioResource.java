...
public class Tbc_relatorio_ensaioResource {
    ...
    public ResponseEntity<List<Tbc_relatorio_ensaio>> searchTbc_relatorio_ensaios(@RequestParam String query, @ApiParam Pageable pageable)
        ...
        Page<Tbc_relatorio_ensaio>  page = null;
        if (query.contains("@r@") || query.contains("@R@") ) {
            String param =  query.replaceAll("@r@","").replaceAll("@R@","");
            page = tbc_relatorio_ensaioService.search(param, true, pageable);
        }else
            page = tbc_relatorio_ensaioService.search(query, false, pageable);
