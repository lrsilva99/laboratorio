...
public class Tbc_clienteResource {
    ...
    public ResponseEntity<List<Tbc_cliente>> searchTbc_clientes(@RequestParam String query, @ApiParam Pageable pageable)
        ...
        Page<Tbc_cliente> page = null;
        if (query.contains("@r@") || query.contains("@R@") ) {
            String param =  query.replaceAll("@r@","").replaceAll("@R@","");
            page = tbc_clienteService.search(param, true, pageable);
        }else
            page = tbc_clienteService.search(query, false, pageable);
